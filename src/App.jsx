...
export default function Dashboard(){
  ...
  var dirStats=useMemo(function(){var totalR=0,totalH=0,rtCount=0;ef.forEach(function(e){var m=gMeta(e);if(m&&m.runtime){totalR+=m.runtime;rtCount++}});totalH=Math.round(totalR/60);var uDir=new Set();ef.forEach(function(e){var m=gMeta(e);if(m&&m.directors)m.directors.split(", ").forEach(function(d){if(d)uDir.add(d)})});return{totalH:totalH,totalR:totalR,uDir:uDir.size,avgRun:rtCount?Math.round(totalR/rtCount):0,rtCount:rtCount,rtTotal:ef.length,rtMissingPct:ef.length?Math.round((ef.length-rtCount)/ef.length*100):0}},[ef,filmMeta]);
  ...
  var TABS_ALL=[{id:'overview',l:'Overview'},{id:'taste',l:'Profile'},{id:'rankings',l:'Rankings'},{id:'people',l:'People'},{id:'where',l:'Where'},{id:'costs',l:'Costs'},{id:'tags',l:'Tags'}];
  ...
  return(<div style={{background:N.paper,color:N.ink,minHeight:'100vh',fontFeatureSettings:'"ss01","cv01"',fontFamily:fontOf('sans')}} className="px-4 md:px-10 py-6 md:py-10"><style>{ANIM_CSS}</style>{pwModal}{themePickerModal}<div className="max-w-6xl mx-auto">
    ...
    {/* FILTER BAR */}
    {['overview','ratings','where','who','yesmine','taste','films','rankings','diary','people'].indexOf(tab)!==-1&&<div className="flex flex-wrap items-center gap-3 mb-6">
    ...
    {tab==='taste'&&<div className="space-y-6">
      ...
      <div><SectionHead T={N} title="Directors" aside={<SrtB T={N} val={sorts.dir} onToggle={function(){ts('dir')}}/>}/><div className="max-h-96 overflow-y-auto"><CTbl T={N} cap={8} data={dirD} sel={sDir} onSel={function(v){sSDir(v)}} sortMode={sorts.dir}/></div></div>
    ...
    {/* ===== FILMS ===== */}
    {tab==='taste'&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-6">
    ...
    {tab==='rankings'&&<div className="space-y-8">
      {top50Evo.years.length>0&&(function(){
        // Columns run newest -> oldest so the current list, the one that matters, is
        // leftmost. Movement in each column is measured against the year to its RIGHT
        // (the chronologically previous list): prev - rank, so a smaller rank reads as
        // a climb regardless of column direction.
        var yrsDisp=top50Evo.years.slice().reverse();
        var latestYear=top50Evo.years[top50Evo.years.length-1];
        var current=top50Evo.films.filter(function(fi){return fi.ranks[latestYear]!==undefined});
        // "Gone" = ever ranked, but absent from the current list. Ordered by how
        // recently they dropped, best last-known rank breaking ties.
        var gone=top50Evo.films.filter(function(fi){return fi.ranks[latestYear]===undefined});
        gone.sort(function(a,b){var la=Math.max.apply(null,Object.keys(a.ranks).map(Number));var lb=Math.max.apply(null,Object.keys(b.ranks).map(Number));if(la!==lb)return lb-la;return(a.ranks[la]||999)-(b.ranks[lb]||999)});
        var mkTable=function(rows){return <div className="overflow-x-auto"><div style={{minWidth:380}}><table className="w-full text-xs"><thead><tr style={{color:N.muted,borderBottom:'0.5px solid '+N.border}}><th className="text-left py-2" style={{fontWeight:400,fontSize:10,letterSpacing:'0.1em',textTransform:'uppercase'}}>Film</th>{yrsDisp.map(function(y){return <th key={y} className="text-center py-2" style={{width:80,fontWeight:400,fontSize:10,letterSpacing:'0.1em',textTransform:'uppercase'}}>{y}</th>})}</tr></thead><tbody>{rows.map(function(fi,i){return <tr key={i} style={{borderBottom:'0.5px solid '+N.border}}><td className="py-1.5" style={{color:N.inkSoft}}><span className="flex items-center gap-2"><Poster meta={gMeta(fi)} w={22}/>{fi.name} <span style={{color:N.muted}}>({fi.year})</span></span></td>{yrsDisp.map(function(y,di){var r=fi.ranks[y];var olderY=yrsDisp[di+1];var hasOlder=olderY!==undefined;var prev=hasOlder?fi.ranks[olderY]:null;var move=r&&prev?(prev-r):null;var isNew=r&&!prev&&hasOlder;var isOut=!r&&prev;return <td key={y} className="py-1.5 text-center"><div className="flex items-center justify-center gap-0.5">{r?<span style={{fontWeight:500,color:N.ink}}>{r}</span>:isOut?<span className="text-xs" style={{color:N.mutedSoft}}>OUT</span>:<span style={{color:N.mutedSoft}}>—</span>}{r&&hasOlder&&(isNew?<span className="ml-0.5" style={{fontSize:10,color:MOVE_NEW,fontWeight:500}}>NEW</span>:move!==null?<span className="ml-0.5" style={{fontSize:10,color:move>0?MOVE_UP:move<0?MOVE_DOWN:N.mutedSoft}}>{move>0?"▲"+move:move<0?"▼"+Math.abs(move):"="}</span>:null)}</div></td>})}</tr>})}</tbody></table></div></div>;};
        return <>
          <div>
            <SectionHead T={N} title="Current Top 50" aside={<span className="text-xs" style={{color:N.mutedSoft}}>most recent first</span>}/>
            {mkTable(current)}
          </div>
          {gone.length>0&&<div>
            <SectionHead T={N} title="Gone, not forgotten" count={gone.length} aside={<span className="text-xs" style={{color:N.mutedSoft,fontStyle:'italic'}}>once ranked, since dropped</span>}/>
            {mkTable(gone)}
          </div>}
        </>;
      })()}
