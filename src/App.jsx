...
if(!SB_URL||!SB_KEY)throw new Error('Missing VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY. Copy .env.example to .env and fill it in.');
...
export default function Dashboard(){
  ...
  var[filmMeta,sFilmMeta]=useState({});var[wlData,sWlData]=useState([]);var[yRatings,sYRatings]=useState({});var[allRatings,sAllRatings]=useState([]);var[top50s,sTop50s]=useState([]);
  ...
  useEffect(function(){Promise.all([sb.from('pipe_data').select('data').eq('id',1).single(),sb.from('tag_registry').select('data').eq('id',1).single(),sb.from('subscriptions').select('data').eq('id',1).single(),sb.from('film_metadata').select('*'),sb.from('watchlist_data').select('data').eq('id',1).single(),sb.from('review_data').select('data').eq('id',1).single(),sb.from('ratings_data').select('data').eq('id',1).single(),sb.from('top50_data').select('*')]).then(function(r){if(r[0].data&&r[0].data.data)sPd(r[0].data.data);if(r[1].data&&r[1].data.data)sReg(r[1].data.data);if(r[2].data&&r[2].data.data&&Array.isArray(r[2].data.data))sSubscriptions(r[2].data.data);if(r[3].data){var fm={};r[3].data.forEach(function(m){fm[m.title+'|||'+m.year]=m});sFilmMeta(fm)}if(r[4].data&&r[4].data.data){try{var wld=typeof r[4].data.data==='string'?JSON.parse(r[4].data.data):r[4].data.data;if(Array.isArray(wld))sWlData(wld)}catch(e){}}if(r[5].data&&r[5].data.data){var revd=r[5].data.data;if(Array.isArray(revd)){var yr={};revd.forEach(function(x){yr[x.name+'|||'+x.year]=x.yRating});sYRatings(yr)}}if(r[6].data&&r[6].data.data){try{var rd=typeof r[6].data.data==='string'?JSON.parse(r[6].data.data):r[6].data.data;if(Array.isArray(rd))sAllRatings(rd)}catch(e){}}if(r[7].data){sTop50s(r[7].data.map(function(x){return{year:x.list_year,films:x.data}}).sort(function(a,b){return a.year-b.year}))}sLoading(false)}).catch(function(){sLoading(false)})},[]);
  ...
  useEffect(function(){var sub=null;sb.auth.getSession().then(function(r){sIsAdmin(!!(r&&r.data&&r.data.session))});var l=sb.auth.onAuthStateChange(function(_evt,session){sIsAdmin(!!session)});if(l&&l.data&&l.data.subscription)sub=l.data.subscription;return function(){if(sub)sub.unsubscribe()}},[]);
...
var diaryData=useMemo(function(){var d=ef.filter(function(e){return!dSrch||e.name.toLowerCase().indexOf(dSrch.toLowerCase())!==-1});return[].concat(d).sort(function(a,b){var c=dSrt.col,m=dSrt.asc?1:-1;if(c==='date')return(a.date<b.date?-1:1)*m;if(c==='name')return a.name.toLowerCase()<b.name.toLowerCase()?-m:m;if(c==='year')return(a.year-b.year)*m;if(c==='rating')return((a.rating||0)-(b.rating||0))*m;return 0})},[ef,dSrt,dSrch]);
  ...
  return(<div style={{background:N.paper,color:N.ink,minHeight:'100vh',fontFeatureSettings:'"ss01","cv01"',fontFamily:fontOf('sans')}} className="px-4 md:px-10 py-6 md:py-10"><style>{ANIM_CSS}</style>{pwModal}{themePickerModal}<div className="max-w-6xl mx-auto">
    ...
    {tab==='diary'&&<div className="space-y-4">
      ...
      <div style={{background:N.surface,border:'0.5px solid '+N.border,borderRadius:4,overflow:'hidden'}}><div className="max-h-screen overflow-y-auto"><table className="w-full text-xs"><thead style={{position:'sticky',top:0,background:N.surfaceAlt}}><tr>{[['date','Date'],['name','Film'],['year','Year'],['rating','\u2605']].map(function(h){var ac=dSrt.col===h[0];return <th key={h[0]} onClick={function(){sDSrt(function(p){return{col:h[0],asc:p.col===h[0]?!p.asc:h[0]==='name'}})}} className="text-left py-2 px-3 cursor-pointer" style={{fontWeight:400,fontSize:10,letterSpacing:'0.1em',textTransform:'uppercase',color:ac?T.primary:N.muted}}>{h[1]}{ac?(dSrt.asc?' \u2191':' \u2193'):''}</th>})}<th className="text-left py-2 px-3" style={{fontWeight:400,fontSize:10,letterSpacing:'0.1em',textTransform:'uppercase',color:N.muted}}>Where</th></tr></thead><tbody>{diaryData.map(function(f,i){return <tr key={i} style={{borderTop:'0.5px solid '+N.border}}><td className="py-1.5 px-3 whitespace-nowrap" style={{color:N.muted}}>{f.date}</td><td className="py-1.5 px-3 truncate max-w-xs" style={{color:N.inkSoft}}>{f.name}{f.rewatch&&<span className="ml-1" style={{color:T.primary}}>{'\u21BB'}</span>}</td><td className="py-1.5 px-3 hidden md:table-cell" style={{color:N.muted}}>{f.year}</td><td className="py-1.5 px-3" style={{color:f.rating?N.ink:N.mutedSoft}}>{f.rating!==null?f.rating+'\u2605':'\u2014'}</td><td className="py-1.5 px-3 truncate max-w-32" style={{color:N.muted}}>{gP(f.tags,fullReg)}</td></tr>})}</tbody></table></div></div>
    </div>}
