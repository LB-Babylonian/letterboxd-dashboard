...
// ============================================================
var THEMES=[
  {id:'neutral',name:'Neutral',dark:false,paper:'#F5F0E6',surface:'#EDE6D8',surfaceAlt:'#E8E0CE',border:'#E0D9C9',borderStrong:'#C9C1AE',ink:'#1A1A1A',inkSoft:'#3A3A38',muted:'#6B6B68',mutedSoft:'#8B8B85',primary:'#1A1A1A',secondary:'#3A3A38',gradient:['#F5F0E6','#EDE6D8','#E8E0CE'],accentTh:'#1A1A1A',accentRt:'#1A1A1A',accentPct:'#6B6B68',dots:['#1A1A1A','#3A3A38','#6B6B68']},
...
var THEME_COPY={
  neutral:{masthead:'Babylonian\u2019s Letterboxd',title:'A year at the movies',heroLabel:'Films watched',heroSuffix:'{n} more than last year',fonts:{hero:'sans',label:'sans',title:'sans'}},
...
var diaryData=useMemo(function(){var d=ef.filter(function(e){return!dSrch||e.name.toLowerCase().indexOf(dSrch.toLowerCase())!==-1});return[].concat(d).sort(function(a,b){var c=dSrt.col,m=dSrt.asc?1:-1;if(c==='date')return(a.date<b.date?-1:1)*m;if(c==='name')return a.name.toLowerCase()<b.name.toLowerCase()?-m:m;if(c==='year')return(a.year-b.year)*m;if(c==='rating')return((a.rating||0)-(b.rating||0))*m;return 0})},[ef,dSrt,dSrch]);
  ...
  var costView=<div className="space-y-6">
    ...
    {monthlyFilt.length>3&&(function(){var sc=seriesColors(T);return <div className="p-4" style={{background:N.surface,border:'0.5px solid '+N.border,borderRadius:0}}><SectionHead T={N} title="Monthly spend"/><ResponsiveContainer width="100%" height={250}><BarChart data={monthlyFilt}><CartesianGrid strokeDasharray="3 3" stroke={N.border}/><XAxis dataKey="m" tick={{fill:N.muted,fontSize:9}} angle={-45} textAnchor="end" height={50}/><YAxis tick={{fill:N.muted,fontSize:10}}/><Tooltip content={function(p){return <CostTip {...p} T={N}/>}}/><Bar dataKey="subs" name="Subscriptions" stackId="a" fill={sc[0]}/><Bar dataKey="tickets" name="Tickets" stackId="a" fill={sc[1]}/><Bar dataKey="rentals" name="Rentals" stackId="a" fill={sc[2]}/></BarChart></ResponsiveContainer><div className="flex gap-4 mt-2 justify-center"><div className="flex items-center gap-1.5"><div style={{width:10,height:10,background:sc[0],borderRadius:0}}/><span className="text-xs" style={{color:N.muted}}>Subscriptions</span></div><div className="flex items-center gap-1.5"><div style={{width:10,height:10,background:sc[1],borderRadius:0}}/><span className="text-xs" style={{color:N.muted}}>Tickets</span></div><div className="flex items-center gap-1.5"><div style={{width:10,height:10,background:sc[2],borderRadius:0}}/><span className="text-xs" style={{color:N.muted}}>Rentals</span></div></div></div>})()}
  </div>;
