...
function fixEnc(s){if(!s)return'';var r=s;try{var P=[['√©','é'],['√®','è'],['√¨','è'],['√ê','ê'],['√†','à'],['√¢','â'],['√Æ','î'],['√ô','ô'],['√ª','û'],['√´','ë'],['√Ø','ï'],['√á','á'],['√ß','ç'],['√±','ñ'],['√ü','ü'],['√Å','Å'],['¬∞','°'],['‚Äì','–'],['‚Äî','—'],['‚Äú','"'],['‚Äù','"'],['‚Äô',"'"],['‚Äò',"'"],['‚Ä¶','…']];for(var i=0;i<P.length;i++){if(r.indexOf(P[i][0])!==-1)r=r.split(P[i][0]).join(P[i][1])}}catch{/* A malformed pair in the table above must not lose the whole string; return it unfixed. */}return r}
...
export default function Dashboard(){
  ...
  var pickTheme=useCallback(function(id){try{localStorage.setItem('dashboard_theme_explicit',id)}catch{/* Private mode denies the write; the theme still applies, it just will not persist. */}sThemeId(id);sShowPicker(false)},[]);
  ...
  // Deliberately silent: this used to set a saving flag that no part of the UI ever read, so a
  ...
  var subsEditorCard=<div>
      <div className="p-4" style={{background:N.surface,border:'0.5px solid '+N.border,borderRadius:4}}>
        ...
        {showSubs&&<div className="space-y-2">{subscriptions.map(function(sub){return <div key={sub.id} className="p-3" style={{background:N.surface,border:'0.5px solid '+N.border,borderRadius:4}}><div className="flex justify-between items-center"><span style={{fontSize:13,fontWeight:500,color:N.ink}}>{sub.name}</span><div className="flex gap-1"><button onClick={function(){sCostEs(costEs===sub.id?null:sub.id)}} style={btnSecondary}>{costEs===sub.id?'Close':'Edit'}</button>{/* Deleting a subscription takes every period with it and writes immediately, and this button
                sits one pixel from Edit -- which is exactly how one got deleted by accident. It now says
                what will be lost and waits for an answer. Terracotta because it is destructive: it used
                to wear the theme accent, the same green as Save, which is no way to dress a delete. */}
            <button onClick={function(){
              var n=(sub.periods||[]).length;
              var msg='Delete the subscription "'+sub.name+'"?\n\nThis also removes '+n+' period'+(n===1?'':'s')+' and cannot be undone.';
              if(confirm(msg)){doUpSubs(function(p){return p.filter(function(s){return s.id!==sub.id})});sCostEs(null)}
            }} style={{background:N.surfaceAlt,border:'0.5px solid '+NEG,borderRadius:4,color:NEG,padding:'4px 10px',fontSize:11}} title={'Delete '+sub.name}>{'\u00D7'}</button></div></div>
